        ...
        methods:{
            formatcityList(cities){
                ...
                for(let i = 0;i < cities.length;i++){
                    //获取城市名称拼音的首字母并大写
                    var firetletter = cities[i].py.substring(0,1).toUpperCase();
                    // console.log(firetletter);
                    if(toCom(firetletter)){ //新添加index
                        cityList.push({index : firetletter, list : [ { nm : cities[i].nm, id : cities[i].id } ]})
                    }else{ //累加已有index中
                        for(let j = 0; j < cityList.length;j++){
                            if( cityList[j].index === firetletter ){
                ...
                function toCom(firetletter){
                    for(let i = 0;i < cityList.length;i++){
                        if(cityList[i].index === firetletter){
                            return false;
                        }
                    }
                    return true;
